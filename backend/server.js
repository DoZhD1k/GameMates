...
app.put("/users/:user_id", (req, res) => {
  ...
  const sql =
    "UPDATE users SET user_name = ?, user_email = ?, user_password = ?, user_role = ? WHERE user_id = ?";
...
app.get("/users", (req, res) => {
  const sql =
    "SELECT user_id, user_name, user_email, user_role, registration_date FROM users";
...
app.get("/games", (req, res) => {
  const sql =
    "SELECT game_id, game_name, game_image, game_tags, game_ranks, game_mods FROM games";
  db.query(sql, (err, results) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: "Failed to fetch users" });
